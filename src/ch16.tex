...
和map一样，通过引用查找值的方法接受任何可以从\texttt{T}借用的类型。细节见“\autoref{borrow}”。
...
在Rust中创建一个新的自定义集合类型和在其他语言中差不多。通过组合语言提供的各个部分来排列数据：结构体和枚举、标准集合、\texttt{Option}、\texttt{Box}等等。例如，“\autoref{GenEnum}”中定义的\texttt{BinaryTree<T>}类型。
