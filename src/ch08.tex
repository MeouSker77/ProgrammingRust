...
\begin{figure}[htbp]
    \centering
    \includegraphics[0.9\textwidth]{../img/f8-2.png}
...
Rust中的文档注释的一个特殊特性是Markdown链接可以使用Rust的item路径而不是相对URL来指明它们引用的item，例如\texttt{leaves::Leaf}。Cargo将会检查路径指向的内容，并创建指向正确文档页面中正确位置的链接。例如，这段代码生成的文档会链接到\texttt{VascularPath}、\texttt{Leaf}和\texttt{Root}的页面：
\begin{minted}{Rust}
