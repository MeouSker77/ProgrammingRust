...
\texttt{writeln!}宏类似于\texttt{println!}宏，除了它把数据写入你指定的流。这里，我们将错误消息写入到了标准错误流\texttt{std::io::stderr}。我们也可以使用\texttt{eprintln!}宏来做同样的事，不过如果中途出错\text{eprintln!}会panic。在\texttt{print\_error}中，我们希望忽略写入消息时出现的错误；我们会在这一章中的“\hyperref[ignoreerr]{忽略错误}”一节中解释原因。
